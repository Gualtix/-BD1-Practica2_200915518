...
SELECT 
    ...
    WHERE (V.fk_hospital IS NOT NULL AND V.fecha_muerte IS NOT NULL)
...
SELECT 
    ...
    WHERE T.nombre = 'Oxigeno' AND V.fk_hospital IS NOT NULL
...
WITH DATA AS(
    ...
    V.apellido       AS Apellido,
    V.direccion      AS Direccion,
    (SELECT COUNT(*) FROM REUNION WHERE REUNION.fk_victima = V.id) AS N_Asociados,
    (SELECT COUNT(*) FROM ASIGNACION_TRATAMIENTO WHERE ASIGNACION_TRATAMIENTO.fk_victima = V.id) AS N_Tratamientos
    FROM VICTIMA V
)
SELECT * FROM DATA WHERE N_Asociados <2 AND N_Tratamientos = 2;
/******************************************************************************************************************************/
/******************************************************************************************************************************/
...
SELECT 
    H.nombre,
    C.nombre,
    COUNT(H.id) AS N_CONTACTO
    FROM HOSPITAL H
    INNER JOIN VICTIMA V ON (H.id = V.fk_hospital)
    INNER JOIN REUNION R ON (V.id = R.fk_victima)
    INNER JOIN DETALLE_REUNION DR ON (R.id = DR.fk_reunion)
    INNER JOIN CONTACTO C ON (DR.fk_contacto) = C.id
