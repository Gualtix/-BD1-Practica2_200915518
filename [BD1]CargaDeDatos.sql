...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            BEGIN
                SELECT id INTO id_hospital FROM HOSPITAL WHERE HOSPITAL.nombre = VICTI.NOMBRE_HOSPITAL AND HOSPITAL.direccion = VICTI.DIRECCION_HOSPITAL;
...
SELECT COUNT(*) FROM DETALLE_REUNION;

EXEC LOAD_ASIGNACION_UBICACIONES;
SELECT COUNT(*) FROM ASIGNACION_UBICACION;

EXEC LOAD_ASIGNACION_TRATAMIENTOS;
SELECT COUNT(*) FROM ASIGNACION_TRATAMIENTO;
/*******************************************************************************************************************************************/

SET SERVEROUTPUT ON;
alter session set nls_date_format='YYYY-MM-DD HH24:MI:SS';

select count(*) from victima  where victima.fk_hospital is not null;
